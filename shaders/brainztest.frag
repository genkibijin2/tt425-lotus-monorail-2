...
void main() {
    ...
    vec2 waver = vec2(0.5, 0.5);
    vec2 u_camera = vec2(waver);
    vec4 v_position = vec4(waver.x, waver.y, uv.x, uv.y);
    vec3 cameraPos = vec3(u_camera.x-uv.x*2.0, u_camera.y-uv.y*2.0, u_camera.x-uv.x*2.0);
...
finalColour = finalColour - vec3(mouseuv.x, mouseuv.y, 0.0);
    finalColour = ceil(finalColour * -0.112) / 0.160;
gl_FragColor = vec4(finalColour, 1.008);
